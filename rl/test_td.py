...
class TestEvaluate(unittest.TestCase):
    ...
    def test_evaluate_finite_mrp(self) -> None:
        ...
        episode_length = 20
        episodes: Iterable[
            Iterable[mp.TransitionStep[bool]]
        ] = self.finite_flip_flop.reward_traces(Choose({True, False}))
        transitions: Iterable[mp.TransitionStep[bool]] = itertools.chain.from_iterable(
    ...
    def test_evaluate_finite_mdp(self) -> None:
        ...
        uniform_policy: mdp.Policy[bool, bool] = mdp.FinitePolicy(
            {s: Choose(self.finite_mdp.actions(s)) for s in self.finite_mdp.states()}
        )

        transitions: Iterable[
            mdp.TransitionStep[bool, bool]
        ...
        q: Optional[Tabular[Tuple[bool, bool]]] = iterate.last(
            cast(Iterator[Tabular[Tuple[bool, bool]]], itertools.islice(qs, 20000))
        )
