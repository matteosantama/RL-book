...
def get_historical_prices(tickers, start, end):
    return pd.concat(
        [pdr.get_data_yahoo(ticker, start, end)["Adj Close"] for ticker in tickers],
        axis=1,
        keys=tickers,
    )
...
if __name__ == "__main__":
    ...
    _, ax = plt.subplots()
    ax.set_xlabel("Standard Deviation of Returns (Annualized)", fontsize=20)
    ax.set_ylabel("Mean Returns (Annualized)", fontsize=20)
    ax.set_title("Historical Returns Mean versus Standard Deviation", fontsize=30)
