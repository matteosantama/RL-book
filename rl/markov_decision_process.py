...
class MarkovDecisionProcess(ABC, Generic[S, A]):
    ...
    def simulate_actions(
        self, start_states: Distribution[S], policy: Policy[S, A]
...
def policy_from_q(
    q: FunctionApprox[Tuple[S, A]], mdp: MarkovDecisionProcess[S, A], ϵ: float = 0.0
) -> Policy[S, A]:
...
class FiniteMarkovDecisionProcess(MarkovDecisionProcess[S, A]):
    ...
    def __init__(self, mapping: StateActionMapping[S, A]):
        ...
        self.non_terminal_states = [s for s, v in mapping.items() if v is not None]
