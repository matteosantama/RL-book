...
from typing import Dict, List, Generic, Optional, Sequence, Tuple, TypeVar, Iterator
...
def unwrap_finite_horizon_MRP(
    ...
    def without_time(
        ...
        return None if arg is None else arg.map(lambda s_r: (s_r[0].state, s_r[1]))

    return [
        {s.state: without_time(process.transition_reward(s)) for s in states}
        for _, states in groupby(sorted(process.states(), key=time), key=time)
    ][:-1]
        ...
        v.append(
            {
                s: res.expectation(
                    lambda s_r: s_r[1]
                    + gamma * (v[-1][s_r[0]] if len(v) > 0 and s_r[0] in v[-1] else 0.0)
                )
...
def unwrap_finite_horizon_MDP(
    ...
    return [
        {s.state: without_time(process.action_mapping(s)) for s in states}
        for _, states in groupby(sorted(process.states(), key=time), key=time)
    ][:-1]
                ...
                action_values = (
                    (
                        res.expectation(
                            ...
                            * (
                                v_p[-1][0][s_r[0]]
                                if len(v_p) > 0 and s_r[0] in v_p[-1][0]
                                else 0.0
                            )
                        ),
                        a,
