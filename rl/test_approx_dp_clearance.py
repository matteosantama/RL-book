...
from rl.approximate_dynamic_programming import (
    backward_evaluate_finite,
    backward_evaluate,
    back_opt_vf_and_policy_finite,
...
class TestEvaluate(unittest.TestCase):
    def setUp(self):
        ...
        self.cp: ClearancePricingMDP = ClearancePricingMDP(
            initial_inventory=ii, time_steps=self.steps, price_lambda_pairs=pairs
        )
    ...
    def test_evaluate_mrp(self):
        ...
        approx_vf_finite = backward_evaluate_finite(
            [(self.mrp_seq[i], fa_dynamic) for i in range(self.steps)], 1.0
        )
        ...
        for t, (v1, v2, v3) in enumerate(zip(vf, approx_vf_finite, approx_vf)):
    ...
    def test_value_iteration(self):
        vpstar = optimal_vf_and_policy(self.mdp_seq, 1.0)
        states = self.single_step_mdp.states()
        fa_dynamic = Dynamic({s: 0.0 for s in states})
        fa_tabular = Tabular()
        distribution = Choose(set(states))
        approx_vpstar_finite = back_opt_vf_and_policy_finite(
            [(self.mdp_seq[i], fa_dynamic) for i in range(self.steps)], 1.0
        )
        ...
        for t, ((v1, _), (v2, _), (v3, _)) in enumerate(
            zip(vpstar, approx_vpstar_finite, approx_vpstar)
        ):
