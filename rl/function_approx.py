...
class FunctionApprox(ABC, Generic[X]):
    ...
    def iterate_updates(
        self, xy_seq_stream: Iterator[Iterable[Tuple[X, float]]]
        ...
        return iterate.accumulate(
            xy_seq_stream, lambda fa, xy: fa.update(xy), initial=self
        )

    def representational_gradient_stream(
        self, x_values_seq: Iterable[X]
    ) -> Iterator[FunctionApprox[X]]:
...
class Dynamic(FunctionApprox[X]):
    ...
    def within(self, other: FunctionApprox[X], tolerance: float) -> bool:
        ...
        return all(
            abs(self.values_map[s] - other.values_map[s]) <= tolerance
...
class Tabular(FunctionApprox[X]):
    ...
    count_to_weight_func: Callable[[int], float] = field(
        default_factory=lambda: lambda n: 1.0 / n
    )
...
class BSplineApprox(FunctionApprox[X]):
    ...
    def representational_gradient(self, x_value: X) -> BSplineApprox[X]:
        ...
        return replace(
            ...
            coeffs=np.array(
                [
                    (
                        BSpline(self.knots, c + one_hots[i] * eps, self.degree)(
                            feature_val
                        )
                        - BSpline(self.knots, c - one_hots[i] * eps, self.degree)(
                            feature_val
    ...
    def update(self, xy_vals_seq: Iterable[Tuple[X, float]]) -> BSplineApprox[X]:
        ...
        sorted_pairs: Sequence[Tuple[float, float]] = sorted(
            zip(feature_vals, y_vals), key=itemgetter(0)
        )
        new_knots, new_coeffs, _ = splrep(
            [f for f, _ in sorted_pairs], [y for _, y in sorted_pairs], k=self.degree
        )
    ...
    def within(self, other: FunctionApprox[X], tolerance: float) -> bool:
        if isinstance(other, BSplineApprox):
            return (
                ...
                and np.all(np.abs(self.coeffs - other.coeffs) <= tolerance).item()
            )
...
class Weights:
    ...
    def update(self, gradient: np.ndarray) -> Weights:
        ...
        new_weights: np.ndarray = (
            self.weights
            - self.adam_gradient.learning_rate
            * corrected_m
...
class LinearFunctionApprox(FunctionApprox[X]):
    ...
    def create(
        ...
        return LinearFunctionApprox(
            ...
            weights=Weights.create(
                adam_gradient=adam_gradient, weights=np.zeros(len(feature_functions))
            )
            if weights is None
            else weights,
    ...
    def regularized_loss_gradient(
        self, xy_vals_seq: Iterable[Tuple[X, float]]
    ) -> np.ndarray:
        x_vals, y_vals = zip(*xy_vals_seq)
        feature_vals: np.ndarray = self.get_feature_values(x_vals)
        diff: np.ndarray = np.dot(feature_vals, self.weights.weights) - np.array(y_vals)
        return (
            np.dot(feature_vals.T, diff) / len(diff)
            + self.regularization_coeff * self.weights.weights
        )

    def update(self, xy_vals_seq: Iterable[Tuple[X, float]]) -> LinearFunctionApprox[X]:
    ...
    def solve(
        ...
        if self.direct_solve:
            ...
            left: np.ndarray = np.dot(
                feature_vals_T, feature_vals
            ) + feature_vals.shape[0] * self.regularization_coeff * np.eye(
            ...
            def done(
                a: LinearFunctionApprox[X], b: LinearFunctionApprox[X], tol: float = tol
            ) -> bool:
                return a.within(b, tol)

            ret = iterate.converged(
                self.iterate_updates(itertools.repeat(xy_vals_seq)), done=done
            )
...
class DNNApprox(FunctionApprox[X]):
    ...
    def create(
        ...
        if weights is None:
            inputs: Sequence[int] = [len(feature_functions)] + [
                n + (1 if dnn_spec.bias else 0) for i, n in enumerate(dnn_spec.neurons)
            ]
    ...
    def forward_propagation(self, x_values_seq: Iterable[X]) -> Sequence[np.ndarray]:
        ...
        for w in self.weights[:-1]:
            ...
            if self.dnn_spec.bias:
                inp = np.insert(out, 0, 1.0, axis=1)
            else:
                inp = out
            ret.append(inp)
        ret.append(
            self.dnn_spec.output_activation(np.dot(inp, self.weights[-1].weights.T))[
                :, 0
            ]
        )
        return ret
    ...
    def within(self, other: FunctionApprox[X], tolerance: float) -> bool:
        if isinstance(other, DNNApprox):
            return all(
                w1.within(w2, tolerance) for w1, w2 in zip(self.weights, other.weights)
            )
        else:
            return False
    ...
    def backward_propagation(
        ...
        deriv: np.ndarray = objective_derivative_output(
            fwd_prop[-1]
        ) * self.dnn_spec.output_activation_deriv(fwd_prop[-1])
        deriv = deriv.reshape(1, -1)
        back_prop: List[np.ndarray] = [np.dot(deriv, layer_inputs[-1]) / deriv.shape[1]]
        ...
        for i in reversed(range(len(self.weights) - 1)):
            ...
            deriv = np.dot(
                self.weights[i + 1].weights.T, deriv
    ...
    def regularized_loss_gradient(
        ...
        def obj_deriv_output(out: np.ndarray) -> np.ndarray:
            return (out - np.array(y_vals)) / self.dnn_spec.output_activation_deriv(out)

        return [
            x + self.regularization_coeff * self.weights[i].weights
            for i, x in enumerate(
                self.backward_propagation(
                    fwd_prop=fwd_prop, objective_derivative_output=obj_deriv_output
                )
            )
        ]

    def update(self, xy_vals_seq: Iterable[Tuple[X, float]]) -> DNNApprox[X]:
        return replace(
            self,
            weights=[
                w.update(g)
                for w, g in zip(
                    self.weights, self.regularized_loss_gradient(xy_vals_seq)
                )
            ],
        )
...
if __name__ == "__main__":
    ...
    ffs = [lambda _: 1.0, lambda x: x[0], lambda x: x[1], lambda x: x[2]]
    ...
    dnna = DNNApprox.create(
        feature_functions=ffs, dnn_spec=ds, adam_gradient=ag, regularization_coeff=0.01
    )
