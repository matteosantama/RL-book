...
from rl.markov_process import TransitionStep, ReturnStep, FiniteMarkovRewardProcess
...
def get_fixed_episodes_from_sr_pairs_seq(
    sr_pairs_seq: Sequence[Sequence[Tuple[S, float]]], terminal_state: S
...
def get_return_steps_from_fixed_episodes(
    fixed_episodes: Sequence[Sequence[TransitionStep[S]]], gamma: float
...
def get_mean_returns_from_return_steps(
    ...
    sorted_returns_seq: Sequence[ReturnStep[S]] = sorted(returns_seq, key=by_state)
    return {
        s: np.mean([r.return_ for r in l])
...
def mc_prediction(
    ...
    return iterate.last(
        itertools.islice(
            mc.mc_prediction(
                traces=episodes_stream, approx_0=Tabular(), γ=gamma, tolerance=1e-10
            ),
            num_episodes,
        )
    ).values_map
...
def finite_mrp(
    ...
    d: Mapping[S, Sequence[Tuple[S, float]]] = {
        s: [(t.next_state, t.reward) for t in l]
        for s, l in itertools.groupby(
            sorted(fixed_experiences, key=by_state), key=by_state
        )
    }
    mrp: Dict[S, Optional[Categorical[Tuple[S, float]]]] = {
        s: Categorical({x: y / len(l) for x, y in collections.Counter(l).items()})
        for s, l in d.items()
    }
...
def td_prediction(
    experiences_stream: Iterator[TransitionStep[S]], gamma: float, num_experiences: int
) -> Mapping[S, float]:
    return iterate.last(
        itertools.islice(
            td.td_prediction(
                ...
                approx_0=Tabular(
                    count_to_weight_func=learning_rate_schedule(
                        initial_learning_rate=0.01, half_life=10000, exponent=0.5
                    )
                ),
                γ=gamma,
            ),
...
if __name__ == "__main__":
    ...
    given_data: Sequence[Sequence[Tuple[str, float]]] = [
        [("A", 2.0), ("A", 6.0), ("B", 1.0), ("B", 2.0)],
        [("A", 3.0), ("B", 2.0), ("A", 4.0), ("B", 2.0), ("B", 0.0)],
        [("B", 3.0), ("B", 6.0), ("A", 1.0), ("B", 1.0)],
        [("A", 0.0), ("B", 2.0), ("A", 4.0), ("B", 4.0), ("B", 2.0), ("B", 3.0)],
        [("B", 8.0), ("B", 2.0)],
    ]
    ...
    fixed_episodes: Sequence[
        Sequence[TransitionStep[str]]
    ] = get_fixed_episodes_from_sr_pairs_seq(
        sr_pairs_seq=given_data, terminal_state="T"
    )

    returns_seq: Sequence[ReturnStep[str]] = get_return_steps_from_fixed_episodes(
        fixed_episodes=fixed_episodes, gamma=gamma
    )

    mean_returns: Mapping[str, float] = get_mean_returns_from_return_steps(returns_seq)
    pprint(mean_returns)

    episodes: Iterator[Sequence[TransitionStep[str]]] = get_episodes_stream(
        fixed_episodes
    )

    mc_pred: Mapping[str, float] = mc_prediction(
        episodes_stream=episodes, gamma=gamma, num_episodes=num_mc_episodes
    )
    pprint(mc_pred)

    fixed_experiences: Sequence[
        TransitionStep[str]
    ...
    experiences: Iterator[TransitionStep[str]] = get_experiences_stream(
        fixed_experiences
    )

    td_pred: Mapping[str, float] = td_prediction(
        experiences_stream=experiences, gamma=gamma, num_experiences=num_td_experiences
    )
    pprint(td_pred)
