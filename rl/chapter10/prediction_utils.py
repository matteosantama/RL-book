from typing import Iterable, TypeVar, Callable, Iterator, Sequence, Tuple, Mapping
...
def mc_prediction_equal_wts(
    ...
    episodes: Iterable[Iterable[TransitionStep[S]]] = mrp_episodes_stream(
        mrp, start_state_distribution
    )
    return mc.mc_prediction(
        traces=episodes, approx_0=initial_func_approx, γ=gamma, tolerance=tolerance
    )
...
def unit_experiences_from_episodes(
    episodes: Iterable[Iterable[TransitionStep[S]]], episode_length: int
...
def td_prediction_learning_rate(
    ...
    return td.td_prediction(
        transitions=td_experiences, approx_0=initial_func_approx, γ=gamma
    )
...
def mc_finite_equal_wts_correctness(
    ...
    mc_vfs: Iterator[FunctionApprox[S]] = mc_finite_prediction_equal_wts(
        fmrp=fmrp, gamma=gamma, tolerance=tolerance, initial_vf_dict=initial_vf_dict
    )
...
def td_finite_learning_rate_correctness(
    ...
    td_vfs: Iterator[FunctionApprox[S]] = td_finite_prediction_learning_rate(
        fmrp=fmrp,
        gamma=gamma,
    ...
    final_td_vf: FunctionApprox[S] = iterate.last(
        itertools.islice(td_vfs, episode_length * num_episodes)
    )
    print(
        "Decaying-Learning-Rate-TD Value Function with " + f"{num_episodes:d} episodes"
    )
...
def compare_td_and_mc(
    ...
    for k, (init_lr, half_life, exponent) in enumerate(learning_rates):
        mc_funcs_it: Iterator[FunctionApprox[S]] = mc_finite_prediction_learning_rate(
            fmrp=fmrp,
            gamma=gamma,
        ...
        for i, mc_f in enumerate(itertools.islice(mc_funcs_it, num_episodes)):
            batch_mc_errs.append(
                sqrt(
                    sum((mc_f(s) - true_vf[j]) ** 2 for j, s in enumerate(states))
                    / len(states)
                )
            )
        ...
        label = (
            f"MC InitRate={init_lr:.3f},HalfLife"
            + f"={half_life:.0f},Exp={exponent:.1f}"
        )
        plt.plot(
            range(len(mc_plot)), mc_plot, color=colors[k], linestyle="-", label=label
        )
    ...
    for k, (init_lr, half_life, exponent) in enumerate(learning_rates):
        td_funcs_it: Iterator[FunctionApprox[S]] = td_finite_prediction_learning_rate(
            fmrp=fmrp,
            gamma=gamma,
        ...
        for i, td_f in enumerate(
            ...
            batch_td_errs.append(
                sqrt(
                    sum((td_f(s) - true_vf[j]) ** 2 for j, s in enumerate(states))
                    / len(states)
                )
            )
        ...
        label = (
            f"TD InitRate={init_lr:.3f},HalfLife"
            + f"={half_life:.0f},Exp={exponent:.1f}"
        )
        plt.plot(
            range(len(td_plot)), td_plot, color=colors[k], linestyle="--", label=label
        )

    plt.xlabel("Episode Batches", fontsize=20)
    plt.ylabel("Value Function RMSE", fontsize=20)
    plt.title("RMSE of MC and TD as function of episode batches", fontsize=25)
    plt.grid(True)
