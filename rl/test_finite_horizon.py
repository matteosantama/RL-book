...
from rl.markov_decision_process import ActionMapping, FiniteMarkovDecisionProcess
...
class TestFiniteMRP(unittest.TestCase):
    ...
    def test_finite_horizon_MRP(self):
        ...
        for state in non_terminal_states:
            expected_transition[state] = Categorical(
                {
        ...
        for state in non_terminal_states:
            distribution.assert_almost_equal(
                self, finite.transition_reward(state), expected_transition[state]
            )
...
class TestFiniteMDP(unittest.TestCase):
    ...
    def test_finite_horizon_MDP(self):
        ...
        expected_result = Categorical(
            {(WithTime(False, time=1), 2.0): 0.7, (WithTime(True, time=1), 1.0): 0.3}
        )
    ...
    def test_unwrap_finite_horizon_MDP(self):
        ...
        def action_mapping_for(
            ...
            return {
                True: Categorical({(same, 1.0): 0.7, (different, 2.0): 0.3}),
                False: Categorical({(same, 1.0): 0.3, (different, 2.0): 0.7}),
            }
        ...
        self.assertEqual(finite.action_mapping(WithTime(state=True, time=10)), None)
