...
class TestEvaluate(unittest.TestCase):
    def setUp(self):
        ...
        self.si_mdp: FiniteMarkovDecisionProcess[
            InventoryState, int
        ...
        self.implied_mrp: FiniteMarkovRewardProcess[
            InventoryState
        ] = self.si_mdp.apply_finite_policy(self.fdp)

        self.states: Sequence[InventoryState] = self.implied_mrp.non_terminal_states

    def test_evaluate_mrp(self):
        mrp_vf1: np.ndarray = self.implied_mrp.get_value_function_vec(self.gamma)
    ...
    def test_value_iteration(self):
        ...
        mdp_fa = iterate.converged(
            value_iteration(
                self.si_mdp, self.gamma, fa, Choose(self.states), num_state_samples=30
            ),
            done=lambda a, b: a.within(b, 0.1),
        )
