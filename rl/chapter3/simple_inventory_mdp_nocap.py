...
class SimpleInventoryMDPNoCap(MarkovDecisionProcess[InventoryState, int]):
    def __init__(
        self, poisson_lambda: float, holding_cost: float, stockout_cost: float
    ):
    ...
    def step(
        self, state: InventoryState, order: int
        ...
        def sample_next_state_reward(
            ...
            reward: float = (
                -self.holding_cost * state.on_hand
    ...
    def fraction_of_days_oos(
        ...
        for _ in range(num_traces):
            steps = itertools.islice(
                impl_mrp.simulate_reward(Constant(start)), time_steps
            )
            for step in steps:
...
class SimpleInventoryDeterministicPolicy(Policy[InventoryState, int]):
    ...
    def act(self, state: InventoryState) -> Constant[int]:
        return Constant(max(self.reorder_point - state.inventory_position(), 0))
