...
def get_logistic_func(alpha: float) -> Callable[[float], float]:
    return lambda x, alpha=alpha: 1.0 / (1 + np.exp(-alpha * x))


# noinspection PyShadowingNames,PyShadowingNames
def get_unit_sigmoid_func(alpha: float) -> Callable[[float], float]:
    return lambda x, alpha=alpha: 1.0 / (
        1 + (1 / np.where(x == 0, VSML, x) - 1) ** alpha
    )
...
if __name__ == "__main__":
    ...
    plot_list_of_curves(
        [x_vals] * len(logistics), y_vals, colors, labels, title="Logistic Functions"
    )
