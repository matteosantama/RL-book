...
def evaluate_finite_mrp(
    mrp: FiniteMarkovRewardProcess[S], γ: float, approx_0: FunctionApprox[S]
    ...
    def update(v: FunctionApprox[S]) -> FunctionApprox[S]:
        ...
        updated: np.ndarray = (
            mrp.reward_function_vec + γ * mrp.get_transition_matrix().dot(vs)
        )
...
def value_iteration_finite(
    mdp: FiniteMarkovDecisionProcess[S, A], γ: float, approx_0: FunctionApprox[S]
) -> Iterator[FunctionApprox[S]]:
    ...
    def update(v: FunctionApprox[S]) -> FunctionApprox[S]:
        ...
        return v.update(
            [
                (s, max(mdp.mapping[s][a].expectation(return_) for a in mdp.actions(s)))
...
def backward_evaluate_finite(
    step_f0_pairs: Sequence[Tuple[RewardTransition[S], FunctionApprox[S]]], γ: float
) -> Iterator[FunctionApprox[S]]:
    ...
    for i, (step, approx0) in enumerate(reversed(step_f0_pairs)):
        ...
        def return_(s_r: Tuple[S, float], i=i) -> float:
            s1, r = s_r
            return r + γ * (
                v[i - 1].evaluate([s1]).item()
                if i > 0 and step_f0_pairs[num_steps - i][0][s1] is not None
                else 0.0
            )
...
MRP_FuncApprox_Distribution = Tuple[
    MarkovRewardProcess[S], FunctionApprox[S], Distribution[S]
]
...
def backward_evaluate(
    ...
    for i, (mrp, approx0, mu) in enumerate(reversed(mrp_f0_mu_triples)):
        ...
        def return_(s_r: Tuple[S, float], i=i) -> float:
            ...
            return r + γ * (
                v[i - 1].evaluate([s1]).item()
                if i > 0 and not mrp_f0_mu_triples[num_steps - i][0].is_terminal(s1)
                else 0.0
            )
...
def back_opt_vf_and_policy_finite(
    ...
    for i, (step, approx0) in enumerate(reversed(step_f0s)):
        ...
        def return_(s_r: Tuple[S, float], i=i) -> float:
            s1, r = s_r
            return r + γ * (
                vp[i - 1][0].evaluate([s1]).item()
                if i > 0 and step_f0s[num_steps - i][0][s1] is not None
                else 0.0
            )
...
MDP_FuncApproxV_Distribution = Tuple[
    MarkovDecisionProcess[S, A], FunctionApprox[S], Distribution[S]
]
...
def back_opt_vf_and_policy(
    ...
    for i, (mdp, approx0, mu) in enumerate(reversed(mdp_f0_mu_triples)):
        ...
        def return_(s_r: Tuple[S, float], i=i) -> float:
            ...
            return r + γ * (
                vp[i - 1][0].evaluate([s1]).item()
                if i > 0 and not mdp_f0_mu_triples[num_steps - i][0].is_terminal(s1)
                else 0.0
            )
...
MDP_FuncApproxQ_Distribution = Tuple[
    MarkovDecisionProcess[S, A], FunctionApprox[Tuple[S, A]], Distribution[S]
]
