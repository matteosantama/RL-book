...
class OptimalExerciseRL:
    ...
    def european_put_price(self, strike: float) -> float:
        ...
        return strike * np.exp(-self.rate * self.expiry) * norm.cdf(
            -d2
        ) - self.spot_price * norm.cdf(-d1)

    def training_sim_data(
        self, num_paths: int, spot_price_frac: float
    ...
    def linear_func_approx(
        self, features: Sequence[Callable[[Tuple[float, float]], float]], reg: float
    ...
    def train_lspi(
        ...
        states: Sequence[Tuple[float, float]] = [
            (i * dt, s) for i, s, _ in training_data
        ]
        ...
        exer: np.ndarray = np.array([self.payoff(t1, s1) for t1, s1 in next_states])
        reg_mat: np.ndarray = fa.regularization_coeff * np.eye(
            len(fa.feature_functions)
        )
        for i in range(training_iters):
            ...
            features_split: Sequence[np.ndarray] = np.array_split(features, split)
            next_features_split: Sequence[np.ndarray] = np.array_split(
                next_features, split, axis=0
            )
            cont_cond_split: Sequence[np.ndarray] = np.array_split(cont_cond, split)
            exer_split: Sequence[np.ndarray] = np.array_split(exer, split)
            for i in range(split):
                a_mat += features_split[i].T.dot(
                    features_split[i]
                    - np.diag(cont_cond_split[i]).dot(next_features_split[i] * gamma)
                )
            ...
            fa = replace(fa, weights=replace(fa.weights, weights=wts))
        return fa
    ...
    def exercise_curve(self, step: int, prices: Sequence[float]) -> np.ndarray:
        t: float = step * self.expiry / self.num_steps
        return np.array([self.payoff(t, p) for p in prices])

    def put_option_exercise_boundary(
        self, func: FunctionApprox[Tuple[float, float]], strike: float
    ) -> Sequence[float]:
        ret: List[float] = []
        prices: np.ndarray = np.arange(0.0, strike + 0.1, 0.1)
        for step in range(self.num_steps):
            cp: np.ndarray = self.continuation_curve(
                func=func, step=step, prices=prices
            )
            ep: np.ndarray = self.exercise_curve(step=step, prices=prices)
            ll: Sequence[float] = [p for p, c, e in zip(prices, cp, ep) if e > c]
            ret.append(max(ll) if len(ll) > 0 else 0.0)
    ...
    def option_price(
        self, scoring_data: np.ndarray, func: FunctionApprox[Tuple[float, float]]
    ) -> float:
...
def fitted_dql_put_option(
    ...
    return obj.train_dql(
        training_data=training_data, init_fa=dnn_approx, training_iters=training_iters
    )
...
def fitted_lspi_put_option(
    ...
    features: List[Callable[[Tuple[float, float]], float]] = [lambda _: 1.0]
    features += [
        (lambda t_s: np.exp(-t_s[1] / (2 * strike)) * lagval(t_s[1] / strike, ident[i]))
        for i in range(num_laguerre)
    ]
    features += [
        lambda t_s: np.cos(-t_s[0] * np.pi / (2 * expiry)),
        lambda t_s: np.log(expiry - t_s[0]) if t_s[0] != expiry else 0.0,
        lambda t_s: (t_s[0] / expiry) ** 2,
    ]

    linear_approx: LinearFunctionApprox[Tuple[float, float]] = obj.linear_func_approx(
        features=features, reg=lspi_reg
    )
...
if __name__ == "__main__":
    ...
    training_data: Sequence[TrainingDataType] = opt_ex_rl.training_sim_data(
        num_paths=num_training_paths, spot_price_frac=spot_price_frac_val
    )
    ...
    for step in [0, int(num_steps_val / 2), num_steps_val - 1]:
        ...
        exer_curve: np.ndarray = opt_ex_rl.exercise_curve(step=step, prices=prices)
        cont_curve_dql: np.ndarray = opt_ex_rl.continuation_curve(
            func=fdql, step=step, prices=prices
        )
        cont_curve_lspi: np.ndarray = opt_ex_rl.continuation_curve(
            func=flspi, step=step, prices=prices
        )
        plt.plot(
            prices,
            cont_curve_dql,
    ...
    scoring_data: np.ndarray = opt_ex_rl.scoring_sim_data(num_paths=num_scoring_paths)

    dql_opt_price: float = opt_ex_rl.option_price(scoring_data=scoring_data, func=fdql)
    print(f"DQL Option Price = {dql_opt_price:.3f}")

    lspi_opt_price: float = opt_ex_rl.option_price(
        scoring_data=scoring_data, func=flspi
    )
