...
class OptimalExerciseBI:
    ...
    def get_mdp(self, t: int) -> MarkovDecisionProcess[StateType, bool]:
        ...
        class OptExerciseBIMDP(MarkovDecisionProcess[StateType, bool]):
            def step(
                ...
                def sr_sampler_func(
                    price_exer=price_exer, exer=exer
                    ...
                    if exercised:
                        ...
                        next_price: float = np.exp(
                            np.random.normal(
                                np.log(price) + (r - s * s / 2) * dt, s * np.sqrt(dt)
                            )
                        )
                        ret = ((next_price, False), 0.0)
                    return ret

                return SampledDistribution(
                    sampler=sr_sampler_func, expectation_samples=200
                )
    ...
    def get_vf_func_approx(
        self, t: int, features: Sequence[Callable[[StateType], float]], reg_coeff: float
    ...
    def backward_induction_vf_and_pi(
        self, features: Sequence[Callable[[StateType], float]], reg_coeff: float
        ...
        mdp_f0_mu_triples: Sequence[
            ...
            (
                self.get_mdp(t=i),
                self.get_vf_func_approx(t=i, features=features, reg_coeff=reg_coeff),
    ...
    def optimal_value_curve(
        self, func: FunctionApprox[StateType], prices: Sequence[float]
    ) -> np.ndarray:
        return func.evaluate([(p, False) for p in prices])

    def exercise_curve(self, prices: Sequence[float]) -> np.ndarray:
        return np.array([self.payoff(p) for p in prices])

    def put_option_exercise_boundary(
        self, opt_vfs: Sequence[FunctionApprox[StateType]], strike: float
    ) -> Sequence[float]:
        ret: List[float] = []
        prices: np.ndarray = np.arange(0.0, strike + 0.1, 0.1)
        for vf in opt_vfs[:-1]:
            cp: np.ndarray = self.optimal_value_curve(func=vf, prices=prices)
        ...
        final: Sequence[Tuple[float, float]] = [(p, self.payoff(p)) for p in prices]
...
if __name__ == "__main__":
    ...
    opt_ex_bi: OptimalExerciseBI = OptimalExerciseBI(
        ...
        payoff=lambda x: max(strike - x, 0.0),
        expiry=expiry_val,
        rate=rate_val,
        vol=vol_val,
    ...
    ffs: List[Callable[[StateType], float]] = [lambda _: 1.0]
    ffs += [
        (
            lambda s_e: np.log(1 + np.exp(-s_e[0] / (2 * strike)))
            * lagval(s_e[0] / strike, ident[i])
        )
        for i in range(num_laguerre)
    ]
    it_vf = opt_ex_bi.backward_induction_vf_and_pi(features=ffs, reg_coeff=reglr_coeff)
